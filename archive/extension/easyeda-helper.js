...
*/
const extensionId = Object.entries(easyeda.extension.instances).filter(
	(e) => e[1].blobURLs && e[1].blobURLs['manifest.json'] == api('getRes', { file: 'manifest.json' }),
)[0][1].id;
	...
	static checkUpdate() {
		(async () => {
			try {
				...
				var cmdUpdatePage = this.createCommand(() => {
					window.open(instance.manifest.homepage, '_blank');
				});
				var cmdUpdateSkip = this.createCommand(() => {
					this.setConfig('update-skip', skipVersion);
				});
