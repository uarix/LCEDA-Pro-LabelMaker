...
var cmdVisitGithub = createCommand(() => {
	...
	menu: [
		{
			text: 'Create Label',
			cmd: createCommand(() => {
				labeldlg.dialog('open');
				labeldlg.dialog('expand');
				reloadFontList();
				previewLabel();
			}),
...
document
	.querySelector(`#extension-${extensionId}-dlg`)
...
function placeLabel() {
	...
	api('editorCall', { cmd: 'getElementById', args: ['root'] }).addEventListener(
		'click',
		(e) => {
			editorAbort();
			$(`#extension-${extensionId}-text`).select();
